...
\section{Préliminaires}
    ...
    Pour illustrer les différentes définitions et leurs utilités, on considerera un feu de circulation.
\subsection{Logique Linéaire Temporelle}
    ...
    On introduit aussi $\psi V \phi = \lnot(\lnot\psi U\lnot\phi)$.
...
\subsection{Automate de Buchi}
    ...
    On peut utiliser l'algorithme de Tarjan par exemple pour trouver les composantes fortement connexe (ou Kosaraju Sharir)
    \begin{algorithm}
    \caption{Cycle Detection}\label{alg:cap}
    \begin{algorithmic}
...
\subsection{Creation de $A_{\lnot\varphi}$}
    ...
    Premièrement, on introduit une structure de donnée: 
    On construit un graphe avec des noeuds décrit par:
    \begin{itemize}
        \item Nom: un string
        \item Incoming: Les arètes qui arrivent sur le noeud représenté par le noeud voisin
        \item New: Un ensemble de formule LTL qui n'ont pas encore été considérer par l'algorithme
        \item Old: Un ensemble de formule LTL qui ont été considéré
        \item Next: Une formule LTL qui doit être vrai pour tout état dans les successeurs immédiats des états qui satisfait les propriétés de Old
        \item Father: Pendant la construction, les noeuds seront divisé, il contient le nom de l'autre noeud avec lequel il a été divisé
    \end{itemize}

    \textbf{Remarque:}
    \begin{itemize}
        \item On notera $Init$ un noeud spécial qui désigne le noeud initial
        \item On note $Nodes\_Set$ une liste de noeud dont la construction est faite 
        \item Father n'est pas necessaire pour l'algorithme, c'est juste pour simplifier les raisonnements et la preuve de correction
    \end{itemize} 
    
    \subsubsection{Algorithm}
    On va supposé sans perte de généralité plusieurs hypothèses sur notre formule:
    \begin{itemize}
        \item L'opérateur $X$ n'est pas utilisé, nous verrons plus tard comment lever cette restriction
        \item Toutes les negations sont poussés sur les variables.
    \end{itemize}
    Ainsi, l'opérateur $V$ est en fait défini pour éviter une explosion exponentielle de la taille de la formule.

    \begin{algorithm}
    \caption{Algorithme de Gerth - Expand}
    \begin{algorithmic}
        \Require $Node, Nodes\_Set$
        \If{$Node.New = \emptyset$}
            \If{$\exists ND\in Nodes\_Set | (ND.Old = Node.Old \land ND.Next = Node.Next)$}
                \State $ND.Incoming \gets ND.Incoming\cup Node.Incoming$
                \State \Return $Nodes\_Set$
            \EndIf
            \State $n \gets new\_name()$
            \State \Return expand([Na:$n$, F:$n$, I:$\{Node.Name\}$, New:$Node.Next$, O:$\emptyset$, Nex:$\emptyset$], $\{Node\}\cup Nodes\_Set$)
        \EndIf
        \State $\eta\in New$
        \State $Node.New := Node.new\setminus\{\eta\}$
        \If{$\eta\in \{F, T, VarProp, \lnot VarProp\}$}
            \If{$(\eta = F) \lor (Neg(\eta)\in Node.Old)$}
                \State \Return $Nodes\_Set$
            \EndIf
            \State $Node.Old := Node.Old\cup\{\eta\}$
            \State \Return expand(Node, $Nodes\_Set$)
        \EndIf
        \If{$(\eta = \varphi \textbf{U} \psi)\lor(\eta = \varphi \textbf{V} \psi)\lor(\eta = \varphi \lor \psi)$}
            \State Node1 := [Na:$new\_name()$, F:$Node.Name$, I:$Node.Incoming$, New:$Node.New \cup New1(\eta) \setminus Node.Old$, O:$Node.Old \cup \{\eta\}$, Nex:$Node.Next\cup Next1(\eta)$]
            \State Node2 := [Na:$new\_name()$, F:$Node.Name$, I:$Node.Incoming$, New:$Node.New \cup New2(\eta) \setminus Node.Old$, O:$Node.Old \cup \{\eta\}$, Nex:$Node.Next$]
            \State \Return expand($Node2$, expand($Node1$, $Nodes\_Set$))
        \EndIf
        \If{$\eta = \varphi \land \psi$}
            \State \Return expand([Na:$Node.Name$, F:$Node.Father$, I:$Node.Incoming$, New:$Node.New \cup \{\varphi, \psi\}\setminus Node.Old$, O:$Node.Old \cup \{\eta\}$, Nex:$Node.Next$], $Nodes\_Set$)
        \EndIf
        \If{$\eta = X\varphi$}
            \State \Return expand([Na:$Node.Name$, F:$Node.Father$, I:$Node.Incoming$, New:$Node.New$, O:$Node.Old \cup \{\eta\}$, Nex:$Node.Next\cup \{\varphi\}$], $Nodes\_Set$)
        \EndIf
    \end{algorithmic}
    \end{algorithm}

    \begin{algorithm}
    \caption{Algorithme de Gerth - Create Graph}
    \begin{algorithmic}
        \Require $\varphi$ 
        \State $n \gets new\_name()$
        \State \Return expand([Name:$n$, Father:$n$, Incoming:$\{Init\}$, New:$\{\varphi\}$, Old:$\emptyset$, Next:$\emptyset$], $\emptyset$)
    \end{algorithmic}
    \end{algorithm}


\subsection{Création de $A_m\times A_{\lnot\varphi}$}
